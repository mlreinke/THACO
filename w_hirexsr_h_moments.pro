...
PRO hmom_save_bins,u
	hirexsr_write_binning,u.shot,chmap=*u.dat.chmap,tch=*u.dat.tch,tmap=u.dat.tmap,good=u.dat.good,tht=u.tht,/h
...
PRO hmom_save_mom,u
	...
	hirexsr_write_moments,u.shot,u.wl.id[line],u.dat.mom[*,*,0:2,line],u.dat.mom[*,*,3:5,line],u.dat.mom[*,*,6:8,line],u.dat.mom[*,*,9:11,line],$
...
PRO hmom_plot_detailed_vessel,u
	x_offset=0.0
	y_offset=0.0
	widget_control,u.id.draw1,get_value=draw_win
	wset,draw_win
...
PRO hmom_plot_cx,u
	hmom_plot_vessel,u
	IF u.stat.fs THEN hmom_plot_fs,u
	IF u.stat.pos THEN hmom_plot_pos,u
END


PRO hmom_plot_all_moments,u
	hmom_plot_spec_moments,u
	IF NOT u.stat.zoom THEN hmom_plot_time_moments,u
	IF u.stat.mconv THEN hmom_plot_conv_moments,u ELSE hmom_plot_spatial_moments,u
END

PRO hmom_plot_time_moments,u
	c=2.998e8 			;speed of light
	e=1.602e-19			;conversion for eV -> J
	mconv=1.661e-27			;conversion for amu -> kg
	mass=39.9477			;mass of argon in amu
	line=u.stat.line
	...
	wset,draw_win
	jgood=u.dat.good[ch,*]
	tmp=where(jgood EQ 1)
	xr=[0,2.0]
	IF u.stat.m0 NE 0 AND tmp[0] NE -1 THEN BEGIN
	...
	IF u.stat.m1 NE 0 AND tmp[0] NE -1 THEN BEGIN
	...
	IF u.stat.m2 NE 0 AND tmp[0] NE -1 THEN BEGIN
		IF u.stat.mplot THEN BEGIN
...
PRO hmom_plot_spatial_moments,u
	c=2.998e8 			;speed of light
	e=1.602e-19			;conversion for eV -> J
	mconv=1.661e-27			;conversion for amu -> kg
	mass=39.9477			;mass of argon in amu
	...
	makesym,10
	;oploterror,ch+1,imom[ch,0],ierr[ch,0],psym=8,color=150,errcolor=150
...
PRO hmom_plot_conv_moments,u

        ;HYDROGEN_LINE_OFFSET = 3; other codes think of lyman alpha 1 as line 3
        LINEID_LYMAN_SERIES = [3, 4, 5] ;  3- lyman, 4-Moly, 5 TJ
        LINE_NUM_USED =3 ;  3 - Lya1, moly, lyman; used for Indices
	widget_control,u.id.draw6,get_value=draw_win
	wset,draw_win
	line=u.stat.line
	lam_o=u.wl.lam_o[line]
	...
	convmom=hirexsr_moment_conv(ispec,icoefs,ilabels,lam_o,LINEID_LYMAN_SERIES(line),dlam)
	!p.multi=[0,0,3]
...
PRO hmom_plot_spec_moments,u	

	;plot background spectra
        ;HYDROGEN_LINE_OFFSET= 3; other codes think of lyman alpha 1 as line 4
        LINEID_LYMAN_SERIES = u.wl.id
	widget_control,u.id.draw4,get_value=draw_win
	wset,draw_win
	line=u.stat.line
	CASE lineid_lyman_series[line] OF
		3 : BEGIN
			wlplot = [3.725, 3.735] ;wlplot=[3.943,3.955]
		END
		4 : BEGIN
			wlplot = [3.735, 3.745];wlplot=[3.962,3.972]
		END
		5 : BEGIN
			wlplot = [3.728, 3.735] ;wlplot = [3.765, 3.775];wlplot=[3.989,4.0]
		END
		6 : BEGIN
			wlplot = [3.170, 3.183]
		END		
	...
	cnts=hirexsr_line_subtract(lam,cnts,icoefs,ilabels,LINEID_LYMAN_SERIES(line))
...
PRO hmom_plot_spec,u
	...
	IF x[1] NE 1 THEN BEGIN
		FOR i=0,(n(icoefs)+1)/3-1 DO oplot,lamfit,gaussian_fits(lamfit,icoefs[i*3:(i+1)*3-1])+last(icoefs),color=80
		oplot,[lamfit[0],last(lamfit)],last(icoefs)*[1,1],color=60
...
PRO update_hmom_temporal_text,u
...
PRO hmom_update_spatial_text,u
	imap=*u.dat.chmap
...
PRO load_hmom_data,u
	shot=u.shot
     	tht=u.tht
	IF hmom_is_dat(u) THEN BEGIN					;if LOAD_DATA already ran, clean out the heap
		cleanheap_w_hirexsr_h_moments,u.id.base
		heap_gc
        ENDIF
        
        morder = [4] ;just for simplicity

	;load raw counts data
	hirexsr_ptr_image,shot,raw,lambda,t,const=const,morder=morder, /h ; load Hlike data
	...
	;load binning data
	hirexsr_load_binning,shot,chmap,tch,tmap,good,chmax, /h,tht=tht		; changed to Hlike
	u.stat.chmax=chmax
	widget_control,u.id.chmax,set_value=num2str(u.stat.chmax,1)

	;load INFO files
	info1=hirexsr_load_info(morder[0],shot=shot,/tree) ;only load one module
	...
	;check to see which crystal is installed
	CASE info.m.bragg.twod OF	
		3.955 : BEGIN		;he-like Ca crystal
   			LINEID_LYMAN_SERIES = [6,-1,-1] ; 6 - he-like Ca w line
		        LINE_NUM_USED =1
			widget_control,u.id.message,set_value='crystal for He-like Ca',/append
			hirexsr_load_wavelengths,shot,lam_o,z,label
			u.wl.lya1=[3.165,3.22]
			u.wl.lam_o[0]=lam_o[where(label EQ 'w' AND z EQ 20)]	;reset data for using the He-like Ca w
			u.wl.id=lineid_lyman_series
		END

		4.56216 : BEGIN		;h-like Ar crystal
   			LINEID_LYMAN_SERIES = [3, 4, 5] ;  3- lyman, 4-Moly, 5 TJ
		        LINE_NUM_USED =3 ;  3 - Lya1, moly, lyman; used for Indices
			widget_control,u.id.message,set_value='crystal for H-like Ar',/append
		END
		ELSE : BEGIN
			widget_control,u.id.message,set_value='unknown TWOD in INFO file',/append
			RETURN
		END
	ENDCASE
	...
	coefs=ptrarr(u.stat.chmax,u.stat.ntime,LINE_NUM_USED,/allocate_heap) ; used to say 3 instead of LINE NUM USED
	labels=ptrarr(LINE_NUM_USED,/allocate_heap)
	FOR i=0,LINE_NUM_USED-1 DO BEGIN
		hirexsr_load_fits,shot,LINEID_LYMAN_SERIES[i],icoefs,nave,double,ilabels,/quiet,status=status,tht=tht
	...
	hirexsr_load_avespec,shot,specbr,lam,sig,resid, /h,tht=tht
	...
	;load moments
	;mom=fltarr(chmax,ntime,15,3)
        mom = fltarr(chmax, ntime, 16, LINE_NUM_USED) ; why 15?
	dlam=fltarr(LINE_NUM_USED)
	pos=fltarr(4,u.stat.chmax,LINE_NUM_USED,u.stat.nmaps)	
	etendue=fltarr(u.stat.chmax,LINE_NUM_USED,u.stat.nmaps)

	FOR i=0,LINE_NUM_USED-1 DO BEGIN	
		IF u.stat.isline[i] THEN hirexsr_load_moments,shot,LINEID_LYMAN_SERIES[i],imom,ierr,ipmom,iperr,tau,ipos,irhot,ifrac,iscale,iu,tpos,idlam,double,ifitcase,tree,/quiet,status=status,tht=tht
		IF status THEN BEGIN
	...
	IF u.stat.display EQ 0 THEN BEGIN		;form spectral bins
		spec=hirexsr_bin_spec(cnts,tau,*chmap,*tch,lambda,tmap,nchbins=u.stat.chmax,const=const)
	...
	u={id:u.id,shot:u.shot,tht:u.tht,time:u.time,index:u.index,mapindex:u.mapindex,ch:u.ch,stat:u.stat,wl:u.wl,dat:dat}
	u.stat.dat=1
	widget_control,u.id.base, set_uvalue=u
END


PRO cleanheap_w_hirexsr_h_moments,widget
...
PRO w_hirexsr_h_moments_event,event
	...
	idtags=tag_names(id)
        ...
        LINEID_LYMAN_SERIES = [3, 4, 5] ;  3- lyman, 4-Moly, 5 TJ
        LINE_NUM_USED =3 ;  3 - Lya1, moly, TJ; used for INdices
	...
	CASE tag OF
		...
		"WIDGET_BUTTON": BEGIN
			...
			CASE ename OF
	 			"QUIT": BEGIN
					cleanheap_w_hirexsr_h_moments,event.top
				...
				"AUTOBIN_R" : BEGIN
					...
					nsub=int(nsub[0])
					IF nsub LE 4 and hmom_is_dat(u) THEN BEGIN
						...
						*u.dat.chmap=chmap
						*u.dat.bnds=bnds
						hmom_update_spatial_text,u
						IF u.stat.pos THEN hmom_plot_cx,u
						IF u.stat.image THEN plot_hmom_image,u
					...
					IF u.stat.dat THEN BEGIN
						...
						*u.dat.chmap=*u.dat.chmap_chk
						IF total(fix) NE 0 THEN BEGIN
							...
							coefs=hirexsr_fit_spectra(avespec,LINEID_LYMAN_SERIES(line),double=u.stat.double,verb=verb,fits=coefs,fix=fix)
							...
							mom[*,*,*,2]=ipmom
							mom[*,*,*,3]=iperr								
							m=hirexsr_spec2moments(u.dat.avespec,u.dat.coefs[*,*,line],*u.dat.labels[line],u.wl.lam_o[line],LINEID_LYMAN_SERIES(line),dlam=u.dat.dlam[line],$
								mom=mom,scale=iscale,fitcase=ifitcase,bfrac=ifrac,fix=fix)
				...
				END
				"MT" : BEGIN
					IF hmom_is_dat(u) AND u.index NE 0 THEN BEGIN
						u.index-=1
						u.time=u.dat.tau[u.index]	
						update_hmom_temporal_text,u
						IF u.stat.pos OR u.stat.fs THEN hmom_plot_cx,u
						IF u.stat.image THEN plot_hmom_image,u
						hmom_plot_spec,u
						hmom_plot_all_moments,u
					ENDIF
				...
				END
				"MCH" : BEGIN
					IF hmom_is_dat(u) AND u.ch NE 1 THEN BEGIN
						u.ch-=1
						hmom_update_spatial_text,u
				...
				"LYA1MOM" : BEGIN
					...
					widget_control,u.id.molymom,set_button=0
					widget_control,u.id.TJmom,set_button=0
					hmom_plot_spec,u
					hmom_plot_all_moments,u
					hmom_update_text,u
				...
				"MOLYMOM" : BEGIN
					...
					widget_control,u.id.lya1mom,set_button=0
					widget_control,u.id.TJmom,set_button=0
					hmom_plot_spec,u
					hmom_plot_all_moments,u
					hmom_update_text,u
				...
				"FIT_ALLSPEC" : BEGIN
					IF u.stat.dat THEN BEGIN
						WIDGET_CONTROL, /hourglass
						line=u.wl.id[u.stat.line]
						coefs=hirexsr_fit_spectra(u.dat.avespec,LINEID_LYMAN_SERIES(line),double=u.stat.double,verb=verb,fix=fix)	;rerun all the fits for the current line
						x=size(fix)
						FOR i=0,x[1]-1 DO FOR j=0,x[2]-1 DO IF fix[i,j] THEN *u.dat.coefs[i,j,line]=*coefs[i,j]
				...
				"CALC_MOM" : BEGIN
					IF u.stat.dat THEN BEGIN
						...
						mom[*,*,*,2]=ipmom
						mom[*,*,*,3]=iperr
						mom=hirexsr_spec2moments(u.dat.avespec,u.dat.coefs[*,*,line],*u.dat.labels[line],u.wl.lam_o[line],LINEID_LYMAN_SERIES(line),dlam=u.dat.dlam[line],$
							mom=mom,scale=iscale,bfrac=ifrac,fix=fix)
						u.dat.mom[*,*,*,line]=[[[mom[*,*,*,0]]],[[mom[*,*,*,1]]],[[mom[*,*,*,2]]],[[mom[*,*,*,3]]],[[irhot]],[[ifrac]],[[iscale]]]
				...
				"CALC_ALLMOM" : BEGIN
					IF u.stat.dat THEN BEGIN
						...
						line=u.wl.id[u.stat.line]
						irhot=u.dat.mom[*,*,12,line]
						mom=hirexsr_spec2moments(u.dat.avespec,u.dat.coefs[*,*,line],*u.dat.labels[line],u.wl.lam_o[line],LINEID_LYMAN_SERIES(line),$
							dlam=u.dat.dlam[line],bfrac=ifrac,scale=iscale)
						u.dat.mom[*,*,*,line]=[[[mom[*,*,*,0]]],[[mom[*,*,*,1]]],[[mom[*,*,*,2]]],[[mom[*,*,*,3]]],[[irhot]],[[ifrac]],[[iscale]]]
			...
			CASE ename OF 
				'CH_SLIDER' : BEGIN
					u.ch=slider
							...
							IF u.stat.pos THEN hmom_plot_cx,u                                                
                                        ...
                                        ENDIF
				END
				'GAIN_SLIDER' : IF hmom_is_dat(u) THEN plot_hmom_image,u
				'T_SLIDER' : BEGIN
					IF hmom_is_dat(u) THEN BEGIN
				...
				'LAM_SLIDER' : BEGIN
					IF hmom_is_dat(u) THEN BEGIN
						...
						hmom_update_text,u
						hmom_plot_spec_moments,u
						IF u.stat.mconv THEN hmom_plot_conv_moments,u
					ENDIF
				END
			ELSE:
			ENDCASE
   		...
   		"WIDGET_TEXT_CH": BEGIN
			CASE event.id OF 
				...
				u.id.vmaxplt : BEGIN
					...
					u.stat.vplt[1]=vmax
					IF u.stat.dat THEN hmom_plot_spatial_moments,u
				END
				u.id.vminplt : BEGIN
...
; This code is based on w_hirexsr_he_moments written by MLR
; It is being modified by YP

PRO w_hirexsr_h_moments,shot=shot,tht=tht
	user=logname()

        MAX_CHANNEL_PER_DET = 32
        PRE_SET_H = 2
        NUM_DET = 1
        print, "This is a BETA release"

	loadct,12,/silent
	base=widget_base(title='HIREXSR H-Like Moment Analysis',/row,tlb_size_events=1, kill_notify= 'cleanheap_w_hirexsr_h_moments')
	...
	ch_slider=widget_slider(B1,ysize=NUM_DET*487*imzoom,min=1,max=MAX_CHANNEL_PER_DET*NUM_DET,value=PRE_SET_H,/drag,/vert)
	B1p1=widget_base(B1,frame=5)
	draw2=widget_draw(B1p1,xsize=195*imzoom,ysize=NUM_DET*487*imzoom)
	...
	lya1mom=widget_button(C4p3,value=' lya1 ')
	molymom=widget_button(C4p3,value=' 4d ')
	TJmom=widget_button(C4p3,value=' TJ ')
	...
	id={base:base,draw1:draw1,draw2:draw2,draw3:draw3,draw4:draw4,draw5:draw5,draw6:draw6,$
		...
		lowpt:lowpt,lam_slider:lam_slider,highpt:highpt,dlampt:dlampt,$
			lya1mom:lya1mom,molymom:molymom,TJmom:TJmom,$
	...
	tplt=[0,2.0]
	vplt=[-10,10]
	stat={dat:0,double:0,image:1,fs:1,pos:1,bins:0,zoom:0,m0:0,m1:1,m2:0,chmax:NUM_DET*MAX_CHANNEL_PER_DET,ntime:0,nmaps:1,maxave:0,line:0,ave:1,display:1,$
		mplot:1,chplot:1,mconv:0,tplt:tplt,vplt:vplt,isline:[0,0,0]}
	hirexsr_load_wavelengths,shot,lam_o,z,label
	tmp=where(z EQ 18)
	lam_o=[lam_o[where(label EQ 'lya1' AND z EQ 18)],lam_o[where(label EQ '4d' AND z EQ 42)],lam_o[where(label EQ 'J' AND z EQ 18)]]
	label=['lya1','4d','TJ']
	wl={lya1:[3.725,3.747], moly:[3.725,3.747],TJ:[3.750,3.78],lam_o:lam_o,label:label,id:[3,4,5]}

	u={id:id,shot:shot,tht:tht,time:time,ch:ch,index:0,mapindex:0,stat:stat,wl:wl}
	...
	CASE u.stat.line OF
		0 : widget_control,u.id.lya1mom,set_button=1
		1 : widget_control,u.id.molymom,set_button=1
		2 : widget_control,u.id.TJmom,set_button=1
		ELSE : 
	END
	...
	xmanager,'w_hirexsr_h_moments',base
END
