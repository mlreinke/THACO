...
pro hirexsr_zoom_plot, v, debug = debug
...
IF slider/1.0e3 GE max(u.dat.tau) THEN index=n(u.dat.tau)
IF slider/1.0e3 LE min(u.dat.tau) THEN index=0
...
function get_channel, v, xnorm, ynorm
...
;	loadct,39,/silent
	...
	;ch=u.ch-1 deleted to allow changing slider position on main widget
        ...
        chval = chmap(channely, channelx)
        pixval = image(channelx, channely)

        returnstr = {chval:chval, channelx:channelx, channely:channely, pixval:pixval}
        return, returnstr
end

; set the selected pixel to ignored
function kill_pixel, v
u = *v.ptru
chmap = *v.ptrinternalchmap
...
IF u.time NE u.index THEN BEGIN
    ...
    ;IF u.stat.image THEN plot_image,u
...
if n_elements(killedpixels) GT 0 then begin
    ...
    if killedpixels(0) NE -1 then begin
        ...
        image(arr_index(1, *), arr_index(0, *)) =0 ; zero out bad pixels
    endif 
endif
...
pro hirexsr_zoom_kill_event, ev
...
case tag OF
    ...
    "WIDGET_DRAW": BEGIN
        ...
        if WIDGET_INFO(v.buttonauto, /button_set) then begin
            ...
            ; now calculate which 
            WIDGET_CONTROL, u.id.ch_slider, get_value =ch
            str_ch = get_channel(v, xnorm, ynorm)
...
pro hirexsr_zoom_kill, ptru, debug=debug

u= *ptru
internalchmap = *u.dat.chmap_chk
...
base = WIDGET_BASE(title = 'HiReX Sr Pixel Removing', /row,tlb_size_events=1, group_leader = u.id.base, /TRACKING_EVENTS, kill_notify = 'hirexsr_zoom_end')
...
; zoom
;zoombase = WIDGET_BASE(textbase, /row)
;zoomlabel = WIDGET_LABEL(zoombase, VALUE = 'Zoom: ', xsize = 40, /align_left)
;zoomtext = WIDGET_TEXT(zoombase, xsize = 10, /editable, VALUE = STRCOMPRESS(STRING(imzoom), /remove_all))
...
;     respondbutton:respondbutton,zoomtext:zoomtext, ptrinternalchmap:ptrinternalchmap,  ptrundoarr:ptrundoarr, ptru:ptru}
v = {base:base, drawid:drawid, buttonauto:buttonauto, buttonman:buttonman,  infozpixtext:infozpixtext, $
     infottext:infottext, infoxpixtext:infoxpixtext, infoypixtext:infoypixtext, killbutton:killbutton, undobutton:undobutton, $
     quitbutton:quitbutton, ptrimzoom:ptrimzoom, plotzoom:plotzoom, imxorig:imxorig, imyorig:imyorig, reloadbutton:reloadbutton, $
...
xmanager, 'hirexsr_zoom_kill', base
