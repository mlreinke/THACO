PRO tifit_update_text,u
	tifit_write_atext,u
	IF NOT u.stat.nob THEN tifit_write_btext,u
	tifit_write_otext,u
	IF u.dat.chk[u.index] THEN widget_control,u.id.chk,set_value='1' ELSE widget_control,u.id.chk,set_value='0'
END

PRO tifit_write_otext,u
	ibs=*u.dat.bsti[u.index]
	widget_control,u.id.timin,set_value=num2str(ibs.limits.(4),dp=2)
	widget_control,u.id.timax,set_value=num2str(ibs.limits.(5),dp=2)
...
PRO tifit_bsfit_index,u
	...
	IF NOT u.stat.nob THEN BEGIN
        ...
        ENDIF ELSE ib=-1
	hirexsr_tifit_input,ia,ib,idat,limits=ibs.limits
	iterfit=0
	conft=0
	CASE ibs.config.fitcase OF
		0 : iterfit=1
	...
	hirexsr_run_bsfit,idat,ibsti,conft=conft,iterfit=iterfit,order=ibs.config.order,nknots=ibs.config.nknots,nsigma=ibs.config.nsigma,$
		nrho=ibs.config.nrho,ntrials=ibs.config.ntrials,fitcase=fitcase
	 *u.dat.bsti[u.index]={fit:ibsti,dat:idat,limits:ibs.limits,error:ibs.error,config:ibs.config,shot:u.shot,tht:u.tht,time:ibs.time}	
END


PRO tifit_plot,u
	tifit_plot_profiles,u
	tifit_plot_ranges,u
END

PRO tifit_plot_ranges,u
...
PRO tifit_plot_profiles,u
	...
	IF u.plot.pauto[1] THEN BEGIN
		...
		ymax=max(ibs.dat.ti[tmp]+ibs.dat.err[tmp]) > max(ibs.fit.prof+ibs.fit.err)
		u.plot.yti=[0,ymax]*1.05
		widget_control,u.id.ytil,set_value=num2str(u.plot.yti[0],dp=2)
		widget_control,u.id.ytih,set_value=num2str(u.plot.yti[1],dp=2)
	ENDIF

	lti=-1.0*ibs.fit.dprof/ibs.fit.prof
	lerr=lti*sqrt((ibs.fit.derr/ibs.fit.dprof)^2+(ibs.fit.err/ibs.fit.prof)^2)
	IF u.plot.pauto[2] THEN BEGIN
		tmp=where(ibs.fit.rho LT 0.8)
		ymin=min(lti[tmp]-lerr[tmp]) < 0
		ymax=max(lti[tmp]+lerr[tmp])
		u.plot.ylti=[ymin,ymax]*1.05
		widget_control,u.id.yltil,set_value=num2str(u.plot.ylti[0],dp=2)
		widget_control,u.id.yltih,set_value=num2str(u.plot.ylti[1],dp=2)
	ENDIF
	
	xr=u.plot.x
	yr=u.plot.yti
	plot,[0],[0],xr=xr,yr=yr,/xsty,/ysty,xtit=ibs.dat.rlab,ytit='Ion Temperature [keV]',chars=ls
	type=[1,2,3,4]
	...
	FOR i=0,n(type) DO BEGIN
		...
		IF tmp[0] NE -1 THEN oploterror,ibs.dat.rho[tmp],ibs.dat.ti[tmp],ibs.dat.err[tmp],psym=3,color=col[i+1],errcolor=col[i+1],thick=dthick
		tmp=where(ibs.fit.good EQ 0 AND ibs.dat.type EQ type[i])
		IF tmp[0] NE -1 THEN oplot,ibs.dat.rho[tmp],ibs.dat.ti[tmp],psym=7,color=col[i+1],thick=dthick
	ENDFOR
	...
	xr=u.plot.x
	yr=u.plot.ylti
	plot,[0],[0],xr=xr,yr=yr,/xsty,/ysty,xtit=ibs.dat.rlab,ytit='Scale Length [a/LTi]',chars=ls
	oploterror,ibs.fit.rho,lti,lerr,color=col[0],errcolor=col[0],thick=fthick
...
PRO tifit_load,u
	IF u.stat.dat THEN cleanup_w_tifit,u
	isdat=0
	runtifit=0
	CASE u.stat.ltree OF 
		0 : BEGIN
			...
			IF NOT chk THEN BEGIN
				...
				IF ask EQ 'Yes' THEN runtifit=1
                        ENDIF ELSE BEGIN
				restore,path
				isdat=1
				widget_control,u.id.message,set_value='BSTI loaded from: '+path,/app
			ENDELSE
                END

		1 : BEGIN
			chk=hirexsr_is_tiresults(u.shot,u.tht,filled=filled)
			IF NOT filled THEN BEGIN
				ask = dialog_message('BSTI data not found, run new analysis?', /question)
				IF ask EQ 'Yes' THEN runtifit=1
                        ENDIF ELSE BEGIN
				hirexsr_read_bsti,u.shot,a,b,c,d,e,tht=u.tht,/ptr,bsti=bsti
				isdat=1
				widget_control,u.id.message,set_value='BSTI loaded from RESULTS node for THT='+num2str(u.tht,1),/app
			ENDELSE	
                END
		ELSE : 
	ENDCASE	
	IF NOT isdat AND NOT runtifit THEN RETURN			;no data and user doesn't want to make new
	IF NOT isdat AND runtifit THEN BEGIN
		hirexsr_tifit,u.shot,tifit,rho,time,err,bsti,tht=u.tht,/ptr,nobranchb=u.stat.nob,inst=u.stat.inst
		widget_control,u.id.message,set_value='new BSTI computed for '+num2str(u.shot,1)+', THT='+num2str(u.tht,1),/app
        ENDIF

	;at this point, either by loading file, loading from tree or running new the bsti ptrarr should be formed
 	;load the raw ADAT and BDAT data which can be used to reformulate the inputs

	hirexsr_tifit_load,u.shot,u.stat.line,adat,atime,tht=u.tht,/rho,inst=u.stat.inst
	widget_control,u.id.message,set_value='ADAT loaded for '+num2str(u.shot,1)+', THT='+num2str(u.tht,1),/app
 	IF NOT u.stat.nob THEN BEGIN
		hirexsr_tifit_load,u.shot,u.stat.line+1,bdat,btime,tht=u.tht,/rho,inst=u.stat.inst
	...
	dat={bsti:bsti,adat:adat,bdat:bdat,time:time,chk:intarr(u.stat.nframe)}
		...
		0 : BEGIN 
			...
			IF chk THEN BEGIN
				...
				IF ask EQ 'No' THEN BEGIN
        	                	widget_control,u.id.message,set_value='BSTI write cancelled',/app
					RETURN
				ENDIF
                        ENDIF 
			save,bsti,time,filename=path
			widget_control,u.id.message,set_value='BSTI written to: '+path,/app
                END

		1 : BEGIN
			chk=0
			IF NOT hirexsr_is_tiresults(u.shot,u.tht) THEN BEGIN
				ask = dialog_message('No RESULTS node for THT='+num2str(u.tht,1)+'  exists. Create new node?', /question)
				IF ask EQ 'Yes' THEN hirexsr_addnew_tiresults,u.shot,tht=u.tht,chk=chk						
                        ENDIF ELSE chk=1
			IF NOT chk THEN BEGIN
				widget_control,u.id.message,set_value='BSTI write cancelled',/app
				RETURN
			ENDIF
			hirexsr_bsfit_ptr2arr,u.dat.bsti,fit,data,config,rlab=rlab
			IF u.stat.inst THEN inst=fit[*,*,0:1]*0.0 ELSE inst=-1	;set INST=0.0 indicating its been removed otherwise flag to be empty
			hirexsr_write_bsti,u.shot,fit,data,inst,config,u.dat.time,rlab=rlab,tht=u.tht
  			widget_control,u.id.message,set_value='BSTI written to RESULTS node for THT='+num2str(u.tht,1),/app
              	END	
	ENDCASE
...
PRO tifit_setpath,u
	path='/home/'+logname()+'/fits/tifit_'+num2str(u.shot,1)+'_THT'+num2str(u.tht,1)+'.dat'
	u.stat.spath=path[0]
	u.stat.lpath=path[0]
	...
	CASE tag OF
		...
		"WIDGET_BUTTON": BEGIN
			...
			CASE ename OF
				...
				"SAVE" : BEGIN
					WIDGET_CONTROL,/hourglass
					IF u.stat.dat THEN tifit_save,u
				END
				...
				"YTIAUTO" : IF event.select EQ 1 THEN u.plot.pauto[1]=1 ELSE u.plot.pauto[1]=0
				"YLTIAUTO" : IF event.select EQ 1 THEN u.plot.pauto[2]=1 ELSE u.plot.pauto[2]=0
				"STREE" : IF event.select EQ 1 THEN u.stat.stree=1 ELSE u.stat.stree=0
				"SFILE" : BEGIN
					out=dialog_pickfile(path='/home/'+logname()+'/fits/',filter='tifit*',/write)
				...
				"LFILE" : BEGIN
					out=dialog_pickfile(path='/home/'+logname()+'/fits/',filter='tifit*',/read,/must_exist)
					IF out NE '' THEN BEGIN
						u.stat.lpath=out
						parts=strsplit(u.stat.lpath,'/',/extract)
						widget_control,u.id.lpath,set_value=last(parts)
				...
				END
				'USE_INST' : BEGIN
					IF event.select EQ 1 THEN u.stat.inst=1 ELSE u.stat.inst=0
					IF u.stat.inst EQ 1 THEN widget_control,u.id.message,set_value='Inst. Temp. from W_HIREXSR_PROFILES Applied',/app
					IF u.stat.inst EQ 0 THEN widget_control,u.id.message,set_value='Inst. Temp. from W_HIREXSR_PROFILES Removed',/app
					widget_control,u.id.message,set_value='Re-Run Spline Fit to Apply',/app
				END
				ELSE :
			ENDCASE
		END
		...
		"WIDGET_TEXT_CH": BEGIN
			...
			END
			u.id.ytil : BEGIN
				u.plot.yti[0]=float(text)
				IF u.stat.dat THEN tifit_plot_profiles,u
			END
			u.id.ytih : BEGIN
				u.plot.yti[1]=float(text)
				IF u.stat.dat THEN tifit_plot_profiles,u
			END
			u.id.yltil : BEGIN
				u.plot.ylti[0]=float(text)
				IF u.stat.dat THEN tifit_plot_profiles,u
			END
			u.id.yltih : BEGIN
				u.plot.ylti[1]=float(text)
				IF u.stat.dat THEN tifit_plot_profiles,u
                             END
			u.id.ainvl : BEGIN
				IF u.stat.dat THEN BEGIN
...
PRO w_hirexsr_tifit,shot=shot,tht=tht,line=line
	font='-adobe-symbol-medium-r-normal--12-120-75-75-p-74-adobe-fontspecific'
	loadct,12,/silent
	mdim=get_screen_size()
	IF mdim[0] LT 1600 OR mdim[1] LT 1000 THEN base=widget_base(title='HIREXSR Ti Fit',/row,tlb_size_events=1,/scroll,$
		x_scroll_size=mdim[0]*0.95,y_scroll_size=mdim[1]*0.85) ELSE base=widget_base(title='HIREXSR Ti Fit',/row,tlb_size_events=1)
	...
	draw1=widget_draw(A1,xsize=700,ysize=425)		
	A2=widget_base(A,frame=5)
	draw2=widget_draw(A2,xsize=700,ysize=425)
	...
	draw3=widget_draw(B,xsize=309,ysize=275,/frame)
	...
	ainve_ev=widget_button(B1p3a, value=' [keV] ')
	...
	alinte_ev=widget_button(B2p3a, value=' [keV] ')
	...
	binve_ev=widget_button(B3p3a, value=' [keV] ')
	...
	blinte_ev=widget_button(B4p3a, value=' [keV] ')	
	
	;instrumental handling
	B5=widget_base(B,xsize=xsize,ysize=50,/column,/frame)
	dum = widget_label(B5,value='INSTRUMETNAL TEMP.')
	B5p1=widget_base(B5,/row,/nonexcl)
	use_inst=widget_button(B5p1,value=' USE PROFILES INST. ')
	...
	timin=widget_text(C2p2,xsize=5,ysize=1,/edit)
	dum = widget_label(C2p2,value=' < Ti < ')
	timax=widget_text(C2p2,xsize=5,ysize=1,/edit)
	dum = widget_label(C2p2,value=' [keV]  ')
	C2p3=widget_base(C2,/row)
	dum = widget_label(C2p3,value='           ')
	dum = widget_label(C2p3,value=' UNC(Ti) < ')
	...
	ytil=widget_text(C5p1,xsize=5,ysize=1,/edit)
	dum=widget_label(C5p1,value='  <  Ti  <  ')
	ytih=widget_text(C5p1,xsize=5,ysize=1,/edit)
	dum=widget_label(C5p1,value=' [keV]  ')
	C5p1a=widget_base(C5p1,/row,/nonexcl)
	ytiauto=widget_button(C5p1a,value='AUTO')
	C5p2=widget_base(C5,/row)
	yltil=widget_text(C5p2,xsize=5,ysize=1,/edit)
	dum=widget_label(C5p2,value='  <  a/LTi  <  ')
	yltih=widget_text(C5p2,xsize=5,ysize=1,/edit)
	C5p2a=widget_base(C5p2,/row,/nonexcl)
	yltiauto=widget_button(C5p2a,value='AUTO')
	...
	id={base:base,draw1:draw1,draw2:draw2,draw3:draw3,$
		...
		timin:timin,timax:timax,emax:emax,nsigma:nsigma,nob:nob,$
		...
		ytil:ytil,ytih:ytih,ytiauto:ytiauto,yltil:yltil,yltih:yltih,yltiauto:yltiauto,xl:xl,xh:xh,xauto:xauto,altplot:altplot}
	...
	time=1.0
	path='/home/'+logname()+'/fits/tifit_'+num2str(shot,1)+'_THT'+num2str(tht,1)+'.dat'
	stat={time:time,dat:0,ps:0,stree:0,ltree:1,inst:0,spath:path[0],lpath:path[0],nob:nob,line:line,nframe:0,pctdef:0.05,evdef:-0.1,estat:[0,0,0,0]}
	plot={col:[50,80,120,150,200],pscol:[30,100,120,150,200],psize:[700,425],rsize:[309,275],pauto:[0,1,1],x:[0.0,1.0],yti:[0.0,3.0],ylti:[0.0,10.0],alt:0}
	...
	widget_control,u.id.ytil,set_value=num2str(u.plot.yti[0],dp=2)
	widget_control,u.id.ytih,set_value=num2str(u.plot.yti[1],dp=2)
	widget_control,u.id.ytiauto,set_button=u.plot.pauto[1]
	widget_control,u.id.yltil,set_value=num2str(u.plot.ylti[0],dp=2)
	widget_control,u.id.yltih,set_value=num2str(u.plot.ylti[1],dp=2)
	widget_control,u.id.yltiauto,set_button=u.plot.pauto[2]
	...
	widget_control,u.id.use_inst,set_button=u.stat.inst
	
	!except=0
	widget_control,base,/realize
	xmanager,'w_hirexsr_tifit',base
